...
import static ru.rzn.gmyasoedov.tetris.core.Figures.FIGURE_G;
import static ru.rzn.gmyasoedov.tetris.core.TestUtils.printMatrix;

class TetrisGCornerTest {

    private static final List<int[][]> POSITIONS = FIGURE_G.getPositions();
    ...
    void toRight3() throws IOException {
        System.out.println(new Object() {}.getClass().getEnclosingMethod().getName());
        int[][] matrix = TestUtils.readMatrix(Paths.get("src/test/resources/empty.txt"));
        Tetris tetris = new Tetris(matrix, new Figure(POSITIONS, 3), 6, 2);
        printMatrix(tetris.getFieldState());
        tetris.toRightInner();
        printMatrix(tetris.getFieldState());
        tetris.toRightInner();
